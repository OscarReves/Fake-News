...
def insert_pd(input_csv,batch_size,queries,batches):    # csv -> database (in batches)
    ...
    for i in range(batches):
        ...
        df = pd.read_csv(input_csv,
                        nrows=batch_size,
                        skiprows=range(1,(i*batch_size)+1+nrows)
                        )
...
def chunk_upload(input_csv,chunk_size,queries,skip_rows=0):    # csv -> database in chunks using inbuilt pandas chunking
    ...
    for i,chunk in enumerate(pd.read_csv(
        ...
        # process chunk to correct format
        values = process(chunk)
        ...
        print((i+1)*chunk_size," rows inserted")
